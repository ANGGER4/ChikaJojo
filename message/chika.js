...
module.exports = async(chika, msg, m, ind, setting) => {
	...
	const isUrl = (uri) => {
        ...
        const buttonsDefault = [
            { callButton: {displayText: `☎ ️Call Owner`, phoneNumber: `+62813-1994-4917`} },
            { urlButton: { displayText: `Script`, url : `https://github.com/GetSya/ChikaJojo`} },
	...
	switch (command) {
            ...
            case prefix+'owner':
                for (let x of ownerNumber) {
                    sendContact(from, x.split('@s.whatsapp.net')[0], 'Owner - Arasya', msg)
                }
            break
            case prefix+'menu': case prefix+'help':{
                textTemplateButtons(from, `Hai kak ${pushname} 👋, saya *${botName}*\n\nBot ini adalah Beta *Multi-Device* Whatsapp.\nMenu : \n - ${prefix}allmenu`, `*© Bot - WhatsApp 2021*\nBy @sofunsyabi.id`, buttonsDefault)
            }
            break
            case prefix+'allmenu': {
                try {
